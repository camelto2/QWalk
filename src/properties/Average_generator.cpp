...
int decide_averager(string & label, Average_generator *& avg) { 
  ...
  else if (caseless_eq(label,"EWALD_CONTRIBUTIONS"))
    avg=new Average_EwaldContributions;
  else 
...
void Average_EwaldContributions::read(System * sys, Wavefunction_data * wfdata, vector <string> & words) { 
}

void Average_EwaldContributions::evaluate(Wavefunction_data * wfdata, Wavefunction * wf,
                              System * sys, Sample_point * sample, Average_return & avg ) { 
  avg.type="ewald_contributions";
  int ndim=4;
  avg.vals.Resize(ndim);
  avg.vals=0.0;
  sys->getEwaldContributions(avg.vals(0),avg.vals(1),avg.vals(2),avg.vals(3));
}

void Average_EwaldContributions::write_init(string & indent, ostream & os) { 
  os << indent << "ewald_contributions \n";
}

void Average_EwaldContributions::read(vector <string> & words) { 
  
}

void Average_EwaldContributions::write_summary(Average_return & avg, Average_return & err, ostream & os) {
  int ndim=avg.vals.GetDim(0);
  assert(ndim <= err.vals.GetDim(0));
  //Could put this in Debye if we want to be nice.
  os << "Ewald contributions contributions (a.u.) \n";
  for(int d=0; d< ndim; d++) { 
    if(d==0) os << "pure_coulomb ";
    else if(d==1) os << "p_nn ";
    else if(d==2) os << "p_en ";
    else if(d==3) os << "p_ee ";
    os << avg.vals(d) << " +/- " << err.vals(d) << endl;
...
void Average_EwaldContributions::jsonOutput(Average_return &avg ,Average_return & err, ostream & os)
{
}
