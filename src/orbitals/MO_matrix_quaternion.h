...
doublevar abs(quaternion x) {
    doublevar tmp = 0.0;
    tmp += abs(x.val.first)*abs(x.val.first);
    tmp += abs(x.val.second)*abs(x.val.second);
    tmp = sqrt(tmp);
    return tmp;
}
...
void MO_matrix_quaternion::updateVal(
  ...
  //R vector is the coordinate space (r,r^2,x,y,z,s)
  static Array1 <doublevar> R(6);
        ...
        for(int i=0; i< imax; i++) {
          ...
          if(R(0) < cutoff(totfunc)) {
            for(int basmo=0; basmo < basismotmp.v[totfunc]; basmo++) {
              ...
              newvals(mo, 0)+=(c.val.first*exp(I*R(5))+c.val.second*exp(-I*R(5)))*symmvals_temp1d(i);
...
void MO_matrix_quaternion::updateLap( Sample_point * sample,
  ...
  //CM
  //newVals needs [val, dx, dy, dz, lap, ds]
  assert(newvals.GetDim(1) >=6);

  // cout << "array " << endl;
  // CM particle corrdinates (r,r2,x,y,z,s)
  Array1 <doublevar> R(6);
  // cout << "symvals " << endl;
   //cout << "arrayref " << endl;
        ...
        if(R(0) < cutoff(totfunc)) {
          for(int basmo=0; basmo < basismotmp.v[totfunc]; basmo++) {
            ...
            for(int j=0; j< 5; j++) {
              ...
              newvals.v[scaleval+j]+=(c.val.first*exp(I*R(5))+c.val.second*exp(-I*R(5)))*symmvals_temp2d.v[scalesymm+j];
...
void MO_matrix_quaternion::updateHessian(
  ...
  Array2 <dcomplex> & newvals
  //!< The return: in form (MO, [val, grad, dxx,dyy,...,ds,dss])
)
{
  ...
  newvals=0;
  assert(e < sample->electronSize());
  //Carries ds,dss at the end
  assert(newvals.GetDim(1)==12);
  

  //Particle corrdinates (r,r2,x,y,z,s)
  Array1 <doublevar> R(6);
 // static Array2 <doublevar> symmvals_temp(maxbasis,10);
  ...
  int b;
  //CM
  //Need+2 to keep the spin derivitives ds, dss
  int symmvals_stride=symmvals_temp2d.GetDim(1)+2;
  for(int ion=0; ion < centermax; ion++)  {
    centers.getDistance(e, ion, R);
    for(int n=0; n< centers.nbasis(ion); n++)  {
      b=centers.basis(ion, n);
